...
public class FunctionalLesson {
     ...
     * <p>
     * сортировка списка целых чисел по сумме цифр числа, затем по значению
     */
...
//    }
     ...
     */
    public void answer42_f(List<String> myStr) {
        myStr.replaceAll(String::toUpperCase);
    }
...
//    }
    ...
    public String fWord_f() throws Exception {
        ...
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(THIS_FILE))
            ...
            result = reader.lines()
                    .flatMap(l -> Stream.of(l.split(WORD_DELIMITERS)))
...
//     *
//     * /
    public static List<Integer> getRNDIntList() {
        final int x = 100;
        final int n = 15;
        Random random = new Random();
        return IntStream.generate(random::nextInt)
                .filter(i -> i <= x && i > 0)
                .limit(random.nextInt(n + 1))
                .boxed()
                .collect(toList());
    }
...
//     *
    ...
    public static List<Integer> getRNDStringList() {
        final int N1 = 15;
        final int N2 = 35;
        final int M1 = 3;
        final int M2 = 8;
